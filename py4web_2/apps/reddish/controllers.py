...
@action.uses("generic.html", db, auth.user) # must be a user to get to this page
def post():
    form = Form(db.entry)
    if form.accepted: # if the Form is accepted then redirect 
        redirect(URL("index"))
    return locals()
